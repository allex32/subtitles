...
Meteor.methods({
    ...
    saveToServer : function(subs, currentVideo, options){
    var file = new Exports(subs, options);
    var srtFile = file.toSRT();
    fs.writeFile(saveSubtitlesDirPath+currentVideo+'.srt',srtFile,function(err){
      if (err)
        console.log(err);
    }); 
  }
